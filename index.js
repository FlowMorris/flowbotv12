...
client.on('voiceStateUpdate', async (oldState, newState) => {
  if (newState.channel != null && newState.channel.name.startsWith('Destek Bekleme')) {newState.guild.channels.create(`🎧 ${newState.member.displayName} Odası`, {type: 'voice',
...
if (oldState.channel != undefined) {
  if (oldState.channel.name.startsWith('🎧')) {
    if (oldState.channel.members.size == 0) {oldState.channel.delete();}
        ...
        let matchMember = oldState.channel.members.find(x => `🎧 ${x.displayName} destek` == oldState.channel.name);
        if (matchMember == null) {
        oldState.channel.setName(`🎧 ${oldState.channel.members.random().displayName} destek`)
          }
       }
     }
   }
